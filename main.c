...
// I2C defines
#define I2C_MASTER MXC_I2C0  //for fuel gauge
...
#define MAX32650_I2C_BUS_3V3_PULLUPS (MXC_I2C1)    //***Change the I2C Bus here
#define MAX32650_I2C_CFG_MASTER_MODE (1)
#define MAX32650_I2C_CLK_SPEED (MXC_I2C_STD_MODE)
...
mxc_gpio_cfg_t gpio_out_1_23;//gpio_out16;
mxc_gpio_cfg_t gpio_out_1_24;//gpio_out20; // adc clock enable
...
//mxc_gpio_cfg_t gpio_in30; // the blue LED on feather (not on motherboard!)  //Not being used
mxc_gpio_cfg_t gpio_out_0_17;//gpio_out12; // pin 4 on feather, used for timing etsts
...
void reset_adc()
{
	...
	gpio_out_1_25.vssel = MXC_GPIO_VSSEL_VDDIO;
...
{
	printf("DMA Call back triggered\n");
}
...
void init_dma_MXC()
{
	...
	dma_config.srcinc_en = 1; // this is ignored??
...
//#endif
	...
	switch(magpie_FS) { // do the correct filter for each sample-rate
		...
		case fs_16k: // timing test, 5ms
			MXC_GPIO_OutSet(gpio_out_2_26.port,gpio_out_2_26.mask); // timing test
			...
			MXC_GPIO_OutClr(gpio_out_2_26.port,gpio_out_2_26.mask); // timing test
			break;
...
{
  if (MXC_I2C_Init(MAX32650_I2C_BUS_3V3_PULLUPS, MAX32650_I2C_CFG_MASTER_MODE, 0) != E_NO_ERROR)
  {
  ...
  const mxc_gpio_cfg_t i2c_1_pins = {
      .port = MXC_GPIO2,
      .mask = (MXC_GPIO_PIN_17 | MXC_GPIO_PIN_18),
  ...
  if (MXC_I2C_SetFrequency(MAX32650_I2C_BUS_3V3_PULLUPS, MAX32650_I2C_CLK_SPEED) != MAX32650_I2C_CLK_SPEED)
  {
  ...
  if(E_NO_ERROR != DS3231_RTC.init(MAX32650_I2C_BUS_3V3_PULLUPS))
  {
...
void MB_LED(u_int8_t state)
{
	...
	// gpio 5 is pin 6 on the feather header, connected to motherboard green LED
		gpio_out_2_26.port =MXC_GPIO_PORT_OUT2;
		gpio_out_2_26.mask= MXC_GPIO_PIN_OUT_2_26;
		gpio_out_2_26.pad = MXC_GPIO_PAD_NONE;
		gpio_out_2_26.func = MXC_GPIO_FUNC_OUT;
		gpio_out_2_26.vssel = MXC_GPIO_VSSEL_VDDIO;
		MXC_GPIO_Config(&gpio_out_2_26);
		MXC_GPIO_OutSet(gpio_out_2_26.port,gpio_out_2_26.mask); // LED OFF
	} else {
		gpio_out_2_26.port =MXC_GPIO_PORT_OUT2;
		gpio_out_2_26.mask= MXC_GPIO_PIN_OUT_2_26;
		gpio_out_2_26.pad = MXC_GPIO_PAD_NONE;
		...
		gpio_out_2_26.vssel = MXC_GPIO_VSSEL_VDDIO;
		MXC_GPIO_Config(&gpio_out_2_26);
		MXC_GPIO_OutClr(gpio_out_2_26.port,gpio_out_2_26.mask); // LED ON
	}
...
// magpie_new - init all the decimation filters. This allows you to change fs without re-compiling
	...
	printf("Initializing SDHC Driver ...\n");
	...
	{
		printf("[OK] --> SDHC driver successfully initialized.\n");
	}
	...
	//////// MAX32650 EV-Kit SDCARD Enabling ////
...
//	}
	...
	printf("Getting Ready to Start Recording ....\n");
...
/* 	// gpio 3 is pin 5 on the feather header and blue LED on motherboard
	...
	// using P2.25 (MAX32650 EV-Kit) instead of P0.3 (MAX32666FTHR)	
	gpio_in_2_25.port =MXC_GPIO_PORT_OUT2;
	gpio_in_2_25.mask= MXC_GPIO_PIN_OUT_2_25;
	gpio_in_2_25.pad = MXC_GPIO_PAD_NONE;
	gpio_in_2_25.func = MXC_GPIO_FUNC_IN;
	gpio_in_2_25.vssel = MXC_GPIO_VSSEL_VDDIO;
	MXC_GPIO_Config(&gpio_in_2_25);
...
//	read_TMR0_regs();
	...
	while(stall) { // stall until a rising edge on slave-sel-B. This is to insure we have no partial writes (1 or 2 bytes) that mess up the dma
		temp1  = MXC_GPIO_InGet(gpio_in_2_25.port,gpio_in_2_25.mask); // L
		temp2 =  MXC_GPIO_InGet(gpio_in_2_25.port,gpio_in_2_25.mask); // H
                 		stall = (!temp2 || temp1);
	...
	printf("Writing to SD Card completed ...\n");
	...
	printf("Writing Meta data to file ...\n\n");
